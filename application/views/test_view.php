...
<h1>Эффективность по клиентам</h1>
<h1>Поиск клиента</h1>
...
= 4px><tr><td>Контактное лицо</td><td>" . $row['full_name'] . "</td></tr>
...
";
        ...
        $sql = "SELECT * FROM contract WHERE client_id = $id ORDER BY cipher";
...
";
   ...
   echo "</table>";
    ...
    if(!empty($sum)) {
        if ($sum != 0)
            echo "<h2>Сумма по всем договорам - $sum</h2>";
